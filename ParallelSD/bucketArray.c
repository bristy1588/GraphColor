...
static int parallel_find_index_max_size(bucket *buckets,
	...
	if (right_index - left_index < GRANULARITY) {
		...
		cilk_spawn int max_left = parallel_find_index_max_size(bucket_array, 
									left_index, 
									(left_index + right_index) / 2);
		int max_right = parallel_find_index_max_size(bucket_array,
...
bucketArray* init_bucketArray(int colors, int max_degree) {
	bucketArray *bucket_array = malloc(sizeof(bucketArray *));
	bucket_array->buckets = malloc(colors * (max_degree + 1) * 
									sizeof(bucket *));
